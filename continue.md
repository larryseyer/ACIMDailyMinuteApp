...
NEXT: Task 5 — Checks, then compile (`tools/verify_video_tab.sh`, then `./build.sh`). Do not start tvOS Read. Do not redo Read or Listen.
...
Four shelves are wired in `ArchiveView`. Do not re-register `VideoPlayableRow` / `VideoTextChapterView`. Do not push a reading from Video.
