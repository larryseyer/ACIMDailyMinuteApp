...
capture_ios() {
    ...
    for pair in "today:01-today" "read:02-read" "lesson:03-lesson" "video:04-video" "saved:05-saved" "settings:06-settings"; do
        tab="${pair%%:*}"
        file="${pair##*:}"
...
for pair in "today:01-today" "read:02-read" "listen:03-listen" "video:04-video"; do
    tab="${pair%%:*}"
    file="${pair##*:}"
    launch_scene "$TV" "$tab"
...
mac_shot video "$OUT/mac/03-video.png"
