// ⛔ tvOS has UserNotifications but cannot DELIVER a local notification —
// it badges only — so the whole reminder stack is fenced out there. The
// names it used to declare now live in NotificationNames.swift, unfenced.
#if !os(tvOS)
import Foundation
